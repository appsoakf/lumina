...
- depends_on: 仅允许引用之前步骤（如 S1、S2），不能引用未来步骤、不能自依赖、不能形成环。
  - 只有“当前步骤必须消费上游输出”时才能添加 depends_on（数据依赖/结论依赖）。
  - 用户原话中的“先/然后/接着/再”等自然语序，不等于依赖关系，不能仅因语序加 depends_on。
  - 对于可独立完成的子任务（如机票搜索与酒店搜索），depends_on 必须是空数组 []。
- input_bindings: 可为空；用于把上游结果映射到当前步骤输入。
  ...
  - to 为当前步骤输入变量名。
  - 仅在当前步骤确实要消费上游字段时填写 input_bindings；否则保持空数组 []。
...
7. 若用户请求包含多个互不依赖的目标，应优先并行规划，而不是强行串行化。
